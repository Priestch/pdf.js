...
import {
  ...
  MAX_SCALE,
  MIN_SCALE,
  noContextMenuHandler, normalizeWheelEventDelta,
  parseQueryString,
...
class PDFViewerApplication {
  constructor(appOptions = new AppOptions()) {
    this.appOptions = appOptions;
    this.bindUtilFunctions();
  ...
  bindExternalServices() {
    if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('FIREFOX || MOZCENTRAL')) {
      ...
      firefoxCom.bindExternalServices(this);
      require('./firefox_print_service.js');
    }
    if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('GENERIC')) {
      const generiCom = require('./genericcom.js');
      generiCom.bindExternalServices(this);
    }
    if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('CHROME')) {
      const chromeCom = require('./chromecom.js');
      chromeCom.bindExternalServices(this);
    }
  ...
  async initialize(appConfig) {
    ...
    if (this.isViewerEmbedded &&
      this.appOptions.get('externalLinkTarget') === LinkTarget.NONE) {
      // Prevent external links from "replacing" the viewer,
      // when it's embedded in e.g. an <iframe> or an <object>.
      this.appOptions.set('externalLinkTarget', LinkTarget.TOP);
    }
  ...
  async _readPreferences() {
    if (this.appOptions.get('disablePreferences') === true) {
    ...
    try {
      ...
      for (const name in prefs) {
        this.appOptions.set(name, prefs[name]);
      }
    } catch (reason) {
  ...
  async _parseHashParameters() {
    if (typeof PDFJSDev !== 'undefined' && PDFJSDev.test('PRODUCTION') &&
      !this.appOptions.get('pdfBugEnabled')) {
      return undefined;
    }
    const waitOn = [];
    ...
    if ('disablerange' in hashParams) {
      this.appOptions.set('disableRange',
                          hashParams['disablerange'] === 'true');
    }
    if ('disablestream' in hashParams) {
      this.appOptions.set('disableStream',
                          hashParams['disablestream'] === 'true');
    }
    if ('disableautofetch' in hashParams) {
      this.appOptions.set('disableAutoFetch',
        hashParams['disableautofetch'] === 'true');
    }
    if ('disablefontface' in hashParams) {
      this.appOptions.set('disableFontFace',
                          hashParams['disablefontface'] === 'true');
    }
    if ('disablehistory' in hashParams) {
      this.appOptions.set('disableHistory',
                          hashParams['disablehistory'] === 'true');
    }
    if ('webgl' in hashParams) {
      this.appOptions.set('enableWebGL', hashParams['webgl'] === 'true');
    }
    if ('useonlycsszoom' in hashParams) {
      this.appOptions.set('useOnlyCssZoom',
                          hashParams['useonlycsszoom'] === 'true');
    }
    if ('verbosity' in hashParams) {
      this.appOptions.set('verbosity', hashParams['verbosity'] | 0);
    }
    if ((typeof PDFJSDev === 'undefined' || !PDFJSDev.test('PRODUCTION')) &&
      hashParams['disablebcmaps'] === 'true') {
      this.appOptions.set('cMapUrl', '../external/cmaps/');
      this.appOptions.set('cMapPacked', false);
    }
    if ('textlayer' in hashParams) {
      switch (hashParams['textlayer']) {
        case 'off':
          this.appOptions.set('textLayerMode', TextLayerMode.DISABLE);
          break;
        case 'visible':
        case 'shadow':
        case 'hover':
    ...
    if ('pdfbug' in hashParams) {
      this.appOptions.set('pdfBug', true);
      let enabled = hashParams['pdfbug'].split(',');
      waitOn.push(this.loadAndEnablePDFBug(enabled));
    }
    // It is not possible to change locale for the (various) extension builds.
    if ((typeof PDFJSDev === 'undefined' ||
      PDFJSDev.test('!PRODUCTION || GENERIC')) && 'locale' in hashParams) {
      this.appOptions.set('locale', hashParams['locale']);
    }
  ...
  async _initializeL10n() {
    this.l10n = this.externalServices.createL10n({
      locale: this.appOptions.get('locale'),
    });
  ...
  async _initializeViewerComponents() {
    ...
    const eventBus = appConfig.eventBus ||
      getGlobalEventBus(this.appOptions.get('eventBusDispatchToDOM'));
    ...
    let pdfLinkService = new PDFLinkService({
      eventBus,
      externalLinkTarget: this.appOptions.get('externalLinkTarget'),
      externalLinkRel: this.appOptions.get('externalLinkRel'),
    });
    this.pdfLinkService = pdfLinkService;

    let downloadManager = this.externalServices.createDownloadManager({
      disableCreateObjectURL: this.appOptions.get('disableCreateObjectURL'),
    });
    ...
    this.pdfViewer = new PDFViewer({
      ...
      downloadManager,
      findController,
      renderer: this.appOptions.get('renderer'),
      enableWebGL: this.appOptions.get('enableWebGL'),
      l10n: this.l10n,
      textLayerMode: this.appOptions.get('textLayerMode'),
      imageResourcesPath: this.appOptions.get('imageResourcesPath'),
      renderInteractiveForms: this.appOptions.get('renderInteractiveForms'),
      enablePrintAutoRotate: this.appOptions.get('enablePrintAutoRotate'),
      useOnlyCssZoom: this.appOptions.get('useOnlyCssZoom'),
      maxCanvasPixels: this.appOptions.get('maxCanvasPixels'),
    });
    ...
    this.pdfCursorTools = new PDFCursorTools({
      container,
      eventBus,
      cursorToolOnLoad: this.appOptions.get('cursorToolOnLoad'),
    });
  ...
  initPassiveLoading() {
    ...
    this.externalServices.initPassiveLoading({
      ...
      onError(err) {
        this.l10n.get('loading_error', null,
  ...
  bindEvents() {
    ...
    eventBus.on('hashchange', this.webViewerHashchange);
    eventBus.on('beforeprint', this.beforePrint);
    eventBus.on('afterprint', this.afterPrint);
    eventBus.on('pagerendered', this.webViewerPageRendered);
    eventBus.on('textlayerrendered', this.webViewerTextLayerRendered);
    eventBus.on('updateviewarea', this.webViewerUpdateViewarea);
    eventBus.on('pagechanging', this.webViewerPageChanging);
    eventBus.on('scalechanging', this.webViewerScaleChanging);
    eventBus.on('rotationchanging', this.webViewerRotationChanging);
    eventBus.on('sidebarviewchanged', this.webViewerSidebarViewChanged);
    eventBus.on('pagemode', this.webViewerPageMode);
    eventBus.on('namedaction', this.webViewerNamedAction);
    eventBus.on('presentationmodechanged', this.webViewerPresentationModeChanged);
    eventBus.on('presentationmode', this.webViewerPresentationMode);
    eventBus.on('openfile', this.webViewerOpenFile);
    eventBus.on('print', this.webViewerPrint);
    eventBus.on('download', this.webViewerDownload);
    ...
    eventBus.on('zoomreset', this.webViewerZoomReset);
    eventBus.on('pagenumberchanged', this.webViewerPageNumberChanged);
    eventBus.on('scalechanged', this.webViewerScaleChanged);
    eventBus.on('rotatecw', this.webViewerRotateCw);
    eventBus.on('rotateccw', this.webViewerRotateCcw);
    eventBus.on('switchscrollmode', this.webViewerSwitchScrollMode);
    eventBus.on('scrollmodechanged', this.webViewerScrollModeChanged);
    eventBus.on('switchspreadmode', this.webViewerSwitchSpreadMode);
    eventBus.on('spreadmodechanged', this.webViewerSpreadModeChanged);
    eventBus.on('documentproperties', this.webViewerDocumentProperties);
    eventBus.on('find', this.webViewerFind);
    eventBus.on('findfromurlhash', this.webViewerFindFromUrlHash);
    eventBus.on('updatefindmatchescount', this.webViewerUpdateFindMatchesCount);
    eventBus.on('updatefindcontrolstate', this.webViewerUpdateFindControlState);
    if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
      eventBus.on('fileinputchange', this.webViewerFileInputChange);
    }
  }
  ...
  bindWindowEvents() {
    ...
    window.addEventListener('wheel', this.webViewerWheel, { passive: false, });
    window.addEventListener('click', this.webViewerClick);
    window.addEventListener('keydown', this.webViewerKeyDown);
  ...
  unbindEvents() {
    ...
    eventBus.off('hashchange', this.webViewerHashchange);
    eventBus.off('beforeprint', this.beforePrint);
    eventBus.off('afterprint', this.afterPrint);
    eventBus.off('pagerendered', this.webViewerPageRendered);
    eventBus.off('textlayerrendered', this.webViewerTextLayerRendered);
    eventBus.off('updateviewarea', this.webViewerUpdateViewarea);
    eventBus.off('pagechanging', this.webViewerPageChanging);
    eventBus.off('scalechanging', this.webViewerScaleChanging);
    eventBus.off('rotationchanging', this.webViewerRotationChanging);
    eventBus.off('sidebarviewchanged', this.webViewerSidebarViewChanged);
    eventBus.off('pagemode', this.webViewerPageMode);
    eventBus.off('namedaction', this.webViewerNamedAction);
    eventBus.off('presentationmodechanged', this.webViewerPresentationModeChanged);
    eventBus.off('presentationmode', this.webViewerPresentationMode);
    eventBus.off('openfile', this.webViewerOpenFile);
    eventBus.off('print', this.webViewerPrint);
    eventBus.off('download', this.webViewerDownload);
    ...
    eventBus.off('zoomreset', this.webViewerZoomReset);
    eventBus.off('pagenumberchanged', this.webViewerPageNumberChanged);
    eventBus.off('scalechanged', this.webViewerScaleChanged);
    eventBus.off('rotatecw', this.webViewerRotateCw);
    eventBus.off('rotateccw', this.webViewerRotateCcw);
    eventBus.off('switchscrollmode', this.webViewerSwitchScrollMode);
    eventBus.off('scrollmodechanged', this.webViewerScrollModeChanged);
    eventBus.off('switchspreadmode', this.webViewerSwitchSpreadMode);
    eventBus.off('spreadmodechanged', this.webViewerSpreadModeChanged);
    eventBus.off('documentproperties', this.webViewerDocumentProperties);
    eventBus.off('find', this.webViewerFind);
    eventBus.off('findfromurlhash', this.webViewerFindFromUrlHash);
    eventBus.off('updatefindmatchescount', this.webViewerUpdateFindMatchesCount);
    eventBus.off('updatefindcontrolstate', this.webViewerUpdateFindControlState);
    if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
      eventBus.off('fileinputchange', this.webViewerFileInputChange);
    }
  }
  ...
  unbindWindowEvents() {
    ...
    window.removeEventListener('click', this.webViewerClick);
    window.removeEventListener('keydown', this.webViewerKeyDown);
  ...
  bindUtilFunctions() {
    if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
      this.validateFileURL = this.__validateFileURL;
      this.webViewerFileInputChange = this.__webViewerFileInputChange;
    }
    this.webViewerOpenFileViaURL = this.__webViewerOpenFileViaURL().bind(this);
    this.webViewerPageRendered = this.__webViewerPageRendered.bind(this);
    this.webViewerTextLayerRendered = this.__webViewerTextLayerRendered.bind(this);
    this.webViewerPageMode = this.__webViewerPageMode.bind(this);
    this.webViewerNamedAction = this.__webViewerNamedAction.bind(this);
    this.webViewerPresentationModeChanged = this.__webViewerPresentationModeChanged.bind(this);
    this.webViewerSidebarViewChanged = this.__webViewerSidebarViewChanged.bind(this);
    this.webViewerUpdateViewarea = this.__webViewerUpdateViewarea.bind(this);
    this.webViewerScrollModeChanged = this.__webViewerScrollModeChanged.bind(this);
    this.webViewerSpreadModeChanged = this.__webViewerSpreadModeChanged.bind(this);
    this.webViewerResize = this.__webViewerResize.bind(this);
    this.webViewerHashchange = this.__webViewerHashchange.bind(this);
    this.webViewerPresentationMode = this.__webViewerPresentationMode.bind(this);
    this.webViewerOpenFile = this.__webViewerOpenFile.bind(this);
    this.webViewerPrint = this.__webViewerPrint.bind(this);
    this.webViewerDownload = this.__webViewerDownload.bind(this);
    this.webViewerFirstPage = this.__webViewerFirstPage.bind(this);
    this.webViewerLastPage = this.__webViewerLastPage.bind(this);
    this.webViewerNextPage = this.__webViewerNextPage.bind(this);
    this.webViewerPreviousPage = this.__webViewerPreviousPage.bind(this);
    this.webViewerZoomIn = this.__webViewerZoomIn.bind(this);
    this.webViewerZoomOut = this.__webViewerZoomOut.bind(this);
    this.webViewerZoomReset = this.__webViewerZoomReset.bind(this);
    this.webViewerPageNumberChanged = this.__webViewerPageNumberChanged.bind(this);
    this.webViewerScaleChanged = this.__webViewerScaleChanged.bind(this);
    this.webViewerRotateCw = this.__webViewerRotateCw.bind(this);
    this.webViewerRotateCcw = this.__webViewerRotateCcw.bind(this);
    this.webViewerSwitchScrollMode = this.__webViewerSwitchScrollMode.bind(this);
    this.webViewerSwitchSpreadMode = this.__webViewerSwitchSpreadMode.bind(this);
    this.webViewerDocumentProperties = this.__webViewerDocumentProperties.bind(this);
    this.webViewerFind = this.__webViewerFind.bind(this);
    this.webViewerFindFromUrlHash = this.__webViewerFindFromUrlHash.bind(this);
    this.webViewerUpdateFindMatchesCount = this.__webViewerUpdateFindMatchesCount.bind(this);
    this.webViewerUpdateFindControlState = this.__webViewerUpdateFindControlState.bind(this);
    this.webViewerScaleChanging = this.__webViewerScaleChanging.bind(this);
    this.webViewerRotationChanging = this.__webViewerRotationChanging.bind(this);
    this.webViewerPageChanging = this.__webViewerPageChanging.bind(this);
    this.webViewerWheel = this.__webViewerWheel.bind(this);
    this.webViewerClick = this.__webViewerClick.bind(this);
    this.webViewerKeyDown = this.__webViewerKeyDown.bind(this);

  }

  __validateFileURL(file) {
  ...
  loadAndEnablePDFBug(enabledTabs) {
  ...
  __webViewerOpenFileViaURL() {
    ...
    return webViewerOpenFileViaURL;
  }

  __webViewerPageRendered(evt) {
  ...
  __webViewerTextLayerRendered(evt) {
    if (typeof PDFJSDev !== 'undefined' &&
      PDFJSDev.test('FIREFOX || MOZCENTRAL') && evt.numTextDivs > 0 &&
      !this.supportsDocumentColors) {
      this.l10n.get('document_colors_not_allowed', null,
  ...
  __webViewerPresentationModeChanged(evt) {
  ...
  __webViewerSidebarViewChanged(evt) {
    this.pdfRenderingQueue.isThumbnailViewEnabled =
      this.pdfSidebar.isThumbnailViewVisible;

    if (this.store && this.isInitialViewSet) {
      // Only update the storage when the document has been loaded *and* rendered.
      this.store.set('sidebarView', evt.view).catch(function () {
      });
    }
  }

  __webViewerUpdateViewarea(evt) {
    ...
    let href = this.pdfLinkService.getAnchorUrl(location.pdfOpenParams);
  ...
  __webViewerScrollModeChanged(evt) {
    let store = this.store;
    if (store && this.isInitialViewSet) {
      // Only update the storage when the document has been loaded *and* rendered.
  ...
  __webViewerSpreadModeChanged(evt) {
    let store = this.store;
    if (store && this.isInitialViewSet) {
      // Only update the storage when the document has been loaded *and* rendered.
  ...
  __webViewerFileInputChange(evt) {
    ...
    if (URL.createObjectURL && !appOptions.get('disableCreateObjectURL')) {
      ...
      fileReader.onload = function webViewerChangeFileReaderOnload(evt) {
        ...
        this.open(new Uint8Array(buffer));
      };
  ...
  __webViewerPresentationMode() {
    this.requestPresentationMode();
  }

  __webViewerOpenFile() {
    if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
      let openFileInputName = this.appConfig.openFileInputName;
  ...
  __webViewerZoomReset() {
    this.zoomReset();
  }

  __webViewerPageNumberChanged(evt) {
    let pdfViewer = this.pdfViewer;
  ...
  __webViewerRotateCcw() {
    this.rotatePages(-90);
  }

  __webViewerSwitchScrollMode(evt) {
    this.pdfViewer.scrollMode = evt.mode;
  }

  __webViewerSwitchSpreadMode(evt) {
    this.pdfViewer.spreadMode = evt.mode;
  }

  __webViewerDocumentProperties() {
  ...
  __webViewerFindFromUrlHash(evt) {
  ...
  __webViewerUpdateFindMatchesCount({ matchesCount, }) {
    if (this.supportsIntegratedFind) {
  ...
  __webViewerUpdateFindControlState({ state, previous, matchesCount, }) {
    if (this.supportsIntegratedFind) {
  ...
  __webViewerScaleChanging(evt) {
    this.toolbar.setPageScale(evt.presetValue, evt.scale);

    this.pdfViewer.update();
  }

  __webViewerRotationChanging(evt) {
  ...
  __webViewerKeyDown(evt) {
    ...
    if (cmd === 1 || cmd === 8 || cmd === 5 || cmd === 12) {
      ...
      switch (evt.keyCode) {
        ...
        case 71: // g
          if (!this.supportsIntegratedFind) {
            let findState = this.findController.state;
            if (findState) {
...
function bindPDFPrintServiceInstance(instance) {
  return PDFPrintServiceFactory.instance = instance;
}

export {
  PDFViewerApplication,
  bindPDFPrintServiceInstance,
